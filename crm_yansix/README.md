...
- Senha inicial: definida em `ADMIN_INITIAL_PASSWORD` nas Script Properties antes do primeiro setup
...
- Configurações persistidas em `CONFIGURACOES` / chave `integrations`.
...
## Fase 15 — Preparação Supabase: usuários, responsáveis e visibilidade
**Data:** 23/08/2026

- Administrador: visão global.
- Gestor: visão dos próprios clientes e dos vendedores ligados a `gestorId`.
- Vendedor: visão apenas dos próprios clientes e dos clientes explicitamente atribuídos a ele.
- Novos clientes passam a usar `usuarios.id` como identificador interno do responsável; registros legados em `responsavel` (nome, e-mail ou início do e-mail) continuam reconhecidos durante a transição.
- A interface exibe o nome do responsável, mas o valor persistido para novos clientes é o ID do usuário.
- O frontend aplica a regra de visibilidade como camada complementar; a proteção definitiva deverá ser aplicada no RLS do Supabase na próxima etapa.
- Criação de novos usuários foi preparada para uma Edge Function administrativa. A `service_role` não é colocada no navegador. Até a configuração do endpoint no Supabase, o CRM informará que o backend administrativo ainda não foi configurado.
- Edição de perfis existentes permanece no CRUD de `public.usuarios`.
- O login continua compatível com a estrutura atual e usa e-mail enquanto o vínculo `auth_user_id` não for adicionado ao banco.

### Próxima etapa no Supabase
1. Criar/ajustar o vínculo seguro entre `auth.users` e `public.usuarios`.
2. Formalizar `responsavelId` (ou decidir manter `responsavel` como ID de usuário).
3. Migrar responsáveis legados para IDs.
4. Criar policies RLS para Administrador, Gestor e Vendedor.
5. Criar a Edge Function administrativa para provisionamento de usuários Auth + perfil CRM.
6. Configurar `ADMIN_USER_FUNCTION_URL` no projeto.
