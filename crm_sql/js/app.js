...
function getResponsavelOptions(){
  ...
  if(SESSION.perfil==="Administrador")return all;
  ...
  // Vendedor só pode ser responsável por si mesmo (o backend também garante isso).
  return all.filter(u=>String(u.id)===String(SESSION.id));
}
function populateResponsavelSelect(el,currentValue){
  if(!el)return;
  const options=getResponsavelOptions();
  const names=new Set(options.map(u=>u.nome));
  let html=`<option value="">Selecione um responsável</option>`+options.map(u=>`<option value="${esc(u.nome)}">${esc(u.nome)}${u.perfil?` · ${esc(u.perfil)}`:""}</option>`).join("");
  const current=String(currentValue||"").trim();
  if(current&&!names.has(current))html+=`<option value="${esc(current)}">${esc(current)} (fora da lista atual)</option>`;
  el.innerHTML=html;
  el.value=current||(SESSION?.nome||"");
