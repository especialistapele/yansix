...
# PostgreSQL / Supabase — nova versão

Esta versão também inclui a migração do backend para PostgreSQL online via Supabase.

Arquivos principais:
- `js/api.js` — camada de comunicação com a API PostgreSQL.
- `js/config.js` — URL da Edge Function.
- `supabase/schema.sql` — estrutura PostgreSQL.
- `supabase/functions/crm-api/index.ts` — API segura do CRM.
- `POSTGRES_SETUP.md` — passo a passo de instalação.

A interface do CRM foi preservada. O Google Sheets deixa de ser necessário para a persistência principal.
