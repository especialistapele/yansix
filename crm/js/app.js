...
function renderUsers(){if(SESSION?.perfil!=="Administrador")return;const rows=[`<div class="user-row user-head"><span>Nome</span><span>E-mail</span><span>Perfil</span><span>Status</span><span>ID</span><span></span></div>`];for(const u of STATE.usuarios||[]){rows.push(`<div class="user-row"><span><strong>${esc(u.nome)}</strong></span><span>${esc(u.email)}</span><span>${esc(u.perfil)}</span><span class="user-status ${u.ativo!==false?"active":"inactive"}">${u.ativo!==false?"Ativo":"Inativo"}</span><span>${esc(String(u.id).slice(0,8))}</span><span class="user-actions"><button class="btn btn-ghost" onclick="openUserModal('${esc(u.id)}')">Editar</button></span></div>`)}document.getElementById("users-table").innerHTML=rows.join("")}
function openUserModal(id=""){if(SESSION?.perfil!=="Administrador")return;const u=(STATE.usuarios||[]).find(x=>String(x.id)===String(id));document.getElementById("user-id").value=u?.id||"";document.getElementById("user-modal-title").textContent=u?"Editar usuário":"Novo usuário";document.getElementById("user-name").value=u?.nome||"";document.getElementById("user-email").value=u?.email||"";document.getElementById("user-password").value="";document.getElementById("user-role").value=u?.perfil||"Vendedor";document.getElementById("user-active").checked=u?.ativo!==false;document.getElementById("user-modal").showModal()}
async function saveUser(e){e.preventDefault();if(SESSION?.perfil!=="Administrador")return;const id=document.getElementById("user-id").value,nome=document.getElementById("user-name").value.trim(),email=document.getElementById("user-email").value.trim().toLowerCase(),senha=document.getElementById("user-password").value,perfil=document.getElementById("user-role").value,ativo=document.getElementById("user-active").checked;try{const duplicate=(STATE.usuarios||[]).find(u=>u.email.toLowerCase()===email&&String(u.id)!==String(id));if(duplicate)throw new Error("Já existe um usuário com este e-mail.");if(!id&&!senha)throw new Error("Informe uma senha para o novo usuário.");const data={nome,email,perfil,ativo};if(senha)data.senha=senha;let saved;if(id)saved=await API.update(CONFIG.SHEETS.USUARIOS,id,data);else saved=await API.create(CONFIG.SHEETS.USUARIOS,{...data,senha,criadoEm:new Date().toISOString()});const i=STATE.usuarios.findIndex(u=>String(u.id)===String(saved.id));if(i>=0)STATE.usuarios[i]={...STATE.usuarios[i],...saved};else STATE.usuarios.push(saved);document.getElementById("user-modal").close();renderUsers();toast("Usuário salvo.");if(String(SESSION.id)===String(saved.id)){SESSION={...SESSION,...saved};localStorage.setItem(CONFIG.SESSION_KEY,JSON.stringify(SESSION));CONFIG.CURRENT_USER=SESSION.nome;applyPermissions()}}catch(err){toast(err.message||"Não foi possível salvar o usuário.","error")}}
...
async function deleteInteraction(id){const i=STATE.interacoes.find(x=>String(x.id)===String(id));if(!i||!confirm("Excluir esta atividade?"))return;try{await API.remove(CONFIG.SHEETS.INTERACOES,id);await auditChange(i.clienteId,"INTERACOES",id,"Exclusão",`Atividade excluída · ${i.tipo||i.canal||""}`);document.getElementById("interaction-modal").close();await syncAll({silent:true});toast("Atividade excluída.")}catch(e){toast(e.message,"error")}}
...
function openDealModal(id=""){
  ...
  document.getElementById("deal-id").value=n?.id||"";document.getElementById("deal-probability").dataset.manual="";document.getElementById("deal-modal-title").textContent=n?"Editar negociação":"Nova negociação";document.getElementById("deal-delete").classList.toggle("hidden",!n);
...
async function deleteDeal(id){const n=STATE.negociacoes.find(x=>String(x.id)===String(id));if(!n||!confirm(`Excluir a negociação de ${clientById(n.clienteId)?.nome||"cliente"}?`))return;try{await API.remove(CONFIG.SHEETS.NEGOCIACOES,id);await auditChange(n.clienteId,"NEGOCIACOES",id,"Exclusão",`Negociação excluída · ${n.produtoServico||"sem produto/serviço"} · ${money(n.valor)}`);document.getElementById("deal-modal").close();await syncAll({silent:true});toast("Negociação excluída.");}catch(e){toast(e.message,"error")}}
...
async function deleteProposal(id){const p=proposalById(id);if(!p||!confirm(`Excluir a proposta ${p.numero||""}?`))return;try{await API.remove(CONFIG.SHEETS.PROPOSTAS,id);await auditChange(p.clienteId,"PROPOSTAS",id,"Exclusão",`Proposta excluída · ${p.numero||""}`);document.getElementById("proposal-modal").close();await syncAll({silent:true});toast("Proposta excluída.")}catch(e){toast(e.message,"error")}}
...
async function deleteTask(id){const t=STATE.tarefas.find(x=>String(x.id)===String(id));if(!t||!confirm(`Excluir a tarefa "${t.titulo}"?`))return;try{await API.remove(CONFIG.SHEETS.TAREFAS,id);await auditChange(t.clienteId,"TAREFAS",id,"Exclusão",`Tarefa excluída: ${t.titulo}`);document.getElementById("task-modal").close();await syncAll({silent:true});toast("Tarefa excluída.");}catch(e){toast(e.message,"error")}}
